...
int set_dst_mac_addr(char *ifname, tcp_packet packet, struct sockaddr_ll *addr) {
  ...
  // set destination mac address to addr->sll_addr
  return arp_request(ifname,
                     src_mac,
                     src_addr,
                     addr->sll_addr,
                     dst_addr);
...
int send_tcp_packet(int sock, char *ifname, tcp_packet *packet) {
  ...
  int ret = set_dst_mac_addr(ifname, *packet, &addr);
  if (ret < 0) {
    return ret;
  }
